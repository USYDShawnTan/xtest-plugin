...
const Plugin_Name = 'xiaotan-plugin'
const Plugin_Path = `${Path}/plugins/${Plugin_Name}`;

if(!Bot.xiaotan_plugin) Bot.xiaotan_plugin = {};

class Config {
	constructor () {
    this.config = {}
	
	Bot.xiaotan_plugin['config'] = this.config;
  ...
  getYaml (app, name, type) {
    ...
    try {
      ...
      logger.error(`[小谈插件][${app}][${name}] 格式错误 ${error}`)
      return false
    }
  ...
  watch (file, app, name, type = 'defSet') {
    ...
    watcher.on('change', path => {
      ...
      logger.mark(`[小谈插件][修改配置文件][${type}][${app}][${name}]`)
